<template>
  <div class="container mx-auto p-4">
    ...
    <form @submit.prevent="signIn" class="max-w-md mx-auto">
      <div class="mb-4">
      ...
      <div class="mb-4">
        ...
        <input v-model="password" type="password" id="password" name="password" autocomplete="current-password" required class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" />
      </div>
      <button type="submit" class="w-full bg-blue-600 text-white py-2 px-4 rounded-md hover:bg-blue-700">Sign In</button>
    </form>

    <button @click="signInWithGoogle" class="mt-4 w-full bg-red-600 text-white py-2 px-4 rounded-md hover:bg-red-700">Sign In with Google</button>
  </div>
</template>
